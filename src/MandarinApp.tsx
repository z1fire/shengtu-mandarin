...
import { splitStudyTime } from "./study-time";
...
import { getGrammarLesson } from "./grammar-lessons";
import {
  buildGradedReading,
...
type PracticeMode = "flashcards" | "recall" | "grammar" | "listening" | "builder" | "reading" | "speaking";
type AppView = "today" | "course" | "library" | "exam" | "progress";
type LibraryView = "words" | "characters" | "grammar";
type GrammarStage = "learn" | "recall";
type RecallFeedback = "question" | "correct" | "remediation";
type RecallExpansion = "example" | "speaking" | null;
  ...
  function startRecording() {
    ...
    recognition.onresult = (event) => {
      ...
      const nextScore = Math.max(
        similarityScore(target, transcript),
        audioTarget && audioTarget !== target ? similarityScore(audioTarget, transcript) : 0,
      );
...
function RecallSpeechPractice({ word, showPinyin, onAttempt }: { word: LevelVocabularyWord; showPinyin: boolean; onAttempt: (correct: boolean) => void }) {
  ...
  const audioTarget = practicingExample ? speechTarget : vocabularySpeechText(word);
...
function mixerSlotGuidance(kind: MixerSlotPart["kind"]) {
  const guidance: Record<MixerSlotPart["kind"], string> = {
    subject: "Who or what the sentence is about",
    noun: "The person or thing that completes the idea",
    verb: "The action or state",
    adjective: "The quality or condition",
    place: "Where the situation happens",
    time: "When the situation happens",
    number: "The quantity or numbered position",
    measure: "The classifier required before this noun",
    modal: "Ability, permission, intention, or likelihood",
    question: "The information you are asking for",
    direction: "The location or direction relationship",
    generic: "A replaceable meaning slot",
  };
  return guidance[kind];
}

function fixedGrammarGuidance(text: string) {
  const meanings: Record<string, string> = {
    "是": "links the subject to an identity",
    "是不是": "asks whether an identity is correct",
    "不": "negates a present, habitual, or descriptive idea",
    "没有": "negates possession or a completed event",
    "有": "expresses having or existence",
    "在": "marks location or where an action happens",
    "的": "links an owner or description to a noun",
    "了": "marks a completed action or new situation",
    "吗": "turns the statement into a yes/no question",
    "呢": "returns or continues the question",
    "也": "adds “also” before the predicate",
    "都": "puts everyone or everything in the same scope",
    "很": "connects a subject naturally to an adjective",
    "太": "marks an excessive or strong degree",
    "请": "makes the request polite",
    "吧": "softens a suggestion or proposal",
    "给": "introduces the recipient",
    "可以": "expresses permission or acceptability",
    "能": "expresses practical ability or possibility",
    "会": "expresses learned ability or a prediction",
    "想": "expresses a wish or intention",
    "要": "expresses a plan, need, or stronger intention",
    "正在": "marks an action in progress right now",
    "一下": "softens or briefly limits the action",
    "第": "turns the following number into an ordinal",
    "，": "separates the two connected parts",
  };
  return meanings[text] ?? "fixed grammar marker — keep it in this position";
}

function GrammarExplanation({ point }: { point: GrammarPoint }) {
  const lesson = getGrammarLesson(point);
  const completeness = lesson.unit === "phrase used inside a sentence"
    ? "The syllabus target itself is a phrase, not a finished thought. The practice frame adds the missing sentence context without changing the target grammar."
    : lesson.unit === "question"
      ? "This pattern forms a complete question when every required slot is filled. Use question intonation and the question mark shown in the model."
      : "This pattern forms a complete statement when every required slot is filled.";

  return <>
    <div className={`grammar-pattern-type grammar-pattern-${lesson.unit.startsWith("phrase") ? "phrase" : lesson.unit.replace(" ", "-")}`}>
      <span>PATTERN TYPE</span><strong>{lesson.unit}</strong><p>{completeness}</p>
    </div>
    <div className="grammar-use-map">
      <span><b>WHAT IT DOES</b>{lesson.purpose}</span>
      <span><b>HOW IT WORKS</b>{lesson.rule}</span>
      <span><b>COMMON MISTAKE</b>{lesson.caution}</span>
    </div>
  </>;
}
...
function GrammarPatternMixer({ point, fallbackFormula, words, onSpeakingAttempt }: { point: GrammarPoint; fallbackFormula: string; words: LevelVocabularyWord[]; onSpeakingAttempt?: (correct: boolean) => void }) {
  ...
  const selectedSentence = mixerExpectedTokens(frame, selections).join("");
  const punctuation = lesson.unit === "question" ? "？" : "。";
  ...
  return <div className="grammar-pattern-mixer">
    <div className="mixer-heading"><span>MIX &amp; MATCH PATTERN LAB</span><strong>Build one complete, meaningful sentence</strong><p>{frame.usesMissionPattern ? "This syllabus row lists related forms rather than one buildable sentence. The lab below is a separate, related mission sentence—not a claim that the whole inventory is one pattern." : "The original target may be only one part of a sentence. This expanded frame supplies the subject, predicate, object, or context needed to make a finished thought."}</p></div>
    <div className="mixer-frame-label"><span>{frame.usesMissionPattern ? "RELATED COMPLETE MISSION FRAME" : "COMPLETE PRACTICE FRAME"}</span><code>{frame.sourceFormula}</code></div>
    <div className="mixer-blueprint"><div><strong>Read the blueprint from left to right</strong><span>White slots change. Yellow grammar pieces stay fixed.</span></div><div className="mixer-blueprint-track">{frame.parts.map((part, index) => <div key={`${part.id}-${index}`} className={part.type === "fixed" ? "fixed" : "slot"}><span>{part.type === "fixed" ? "KEEP" : "CHOOSE"}</span><b>{part.type === "fixed" ? part.text : part.label}</b><small>{part.type === "fixed" ? fixedGrammarGuidance(part.text) : mixerSlotGuidance(part.kind)}</small></div>)}</div></div>
    ...
    <div className="mixer-live-model"><span>YOUR COMPLETE WORKING {lesson.unit === "question" ? "QUESTION" : "SENTENCE"}</span><strong lang="zh-CN">{selectedSentence}{punctuation}</strong><div>{slots.map((slot) => { const word = slot.options[selections[slot.id] ?? 0]; return word ? <small key={slot.id}><b>{word.hanzi}</b> {word.pinyin} · {word.meaning}</small> : null; })}</div><p>Choose the meaning you want first. Then keep the fixed pieces in place and arrange every tile in the blueprint order.</p></div>
...
export default function MandarinApp() {
  ...
  const [recallFeedback, setRecallFeedback] = useState<RecallFeedback>("question");
  const [recallExpansion, setRecallExpansion] = useState<RecallExpansion>(null);
  const [recallMissedIndices, setRecallMissedIndices] = useState<number[]>([]);
  const [recallSpeakingRound, setRecallSpeakingRound] = useState(false);
  ...
  const [correctionResultFor, setCorrectionResultFor] = useState("");
  ...
  useEffect(() => {
    ...
    const commitPendingTime = () => {
      ...
      setProgress((current) => {
        const next = recordActiveStudySeconds(current, seconds, recordedAt);
  ...
  const sessionFlashcardQueue = replaySession?.vocabularyQueue ?? progress.dailyQueue;
  const sessionRecallQueue = replaySession?.recallQueue ?? (repeatingCurrentRecall ? currentRecallReplayQueue : progress.recallQueue);
  const sessionVocabularyQueue = practice === "flashcards" ? sessionFlashcardQueue : sessionRecallQueue;
  const sessionFlashcardPosition = replaySession?.flashcardPosition ?? progress.flashcardPosition;
  ...
  const activeCorrectionAudioWord = activeCorrectionAudio ? vocabulary.find((word) => word.hanzi === activeCorrectionAudio) : undefined;
  ...
  const activeVocabularyPosition = practice === "flashcards" ? sessionFlashcardPosition : sessionCardPosition;
  const activeWordIndex = sessionVocabularyQueue[activeVocabularyPosition];
  ...
  const speakingOnlyRecall = repeatingCurrentRecall && recallSpeakingRound;
  const testingRecall = practice === "recall" && !speakingOnlyRecall;
  ...
  const recallPromptHasActions = testingRecall && recallFeedback === "question" && !cardRevealed;
  const showRecallPinyin = recallFeedback === "remediation" ? true : showActivePinyin;
  const nextCadenceDays = Math.max(1, (activeReview?.intervalDays ?? 0) + 1);
  const queuePercent = sessionVocabularyQueue.length ? Math.round((activeVocabularyPosition / sessionVocabularyQueue.length) * 100) : 100;
  ...
  const missionReady = ["flashcards", "review", "grammar", "listen", "build", "reading"].every((step) => sessionDaily.includes(step));
  ...
  const examAudioWord = vocabulary.find((word) => word.hanzi === examQuestion.prompt);
  ...
  const dailySteps = [
    { id: "flashcards", mode: "flashcards" as const, time: 5, title: "Learn the cards", detail: sessionVocabularyQueue.length ? `${sessionVocabularyQueue.length} scheduled words · full answers` : "Queue complete", accent: "coral" },
    { id: "review", mode: "recall" as const, time: 5, title: "Recall check", detail: sessionVocabularyQueue.length ? `${sessionVocabularyQueue.length} scheduled words · answers hidden` : "Queue complete", accent: "blue" },
  ...
  const practiceOrder: PracticeMode[] = ["flashcards", "recall", "grammar", "listening", "builder", "reading", "speaking"];
  const practiceLabels: Record<PracticeMode, string> = { flashcards: "Flashcards", recall: "Recall", grammar: "Grammar", listening: "Listening", builder: "Sentence lab", reading: "Reading", speaking: "Mission speaking" };
  ...
  function answerRecallChallenge(option: string) {
    if (!recallChallenge || activeWordIndex === undefined || !activeWord || recallAdvancingRef.current || recallFeedback !== "question") return;
    const correct = option === recallChallenge.answer;
    registerRecallAttempt(correct);
    if (!correct) {
      openRecallRemediation("Miss recorded · study the complete card, say it aloud, then retry.");
      return;
    }
  ...
  function requestRecallHelp() {
    if (!recallChallenge || activeWordIndex === undefined || !activeWord || recallFeedback !== "question") return;
    registerRecallAttempt(false);
    openRecallRemediation("Miss recorded · study the answer and retry it from memory.");
  }
  ...
  function answerCorrection(option: string) {
    ...
    const answeredCorrectionId = activeCorrection.id;
    setCorrectionResultFor(answeredCorrectionId);
  ...
  function startStudyDayReplay(day: StudyDay) {
    ...
    const replayVocabularyQueues = buildRandomizedVocabularyQueues(vocabularyQueue);
    ...
    const completedSteps = [
      ...(vocabularyQueue.length ? [] : ["flashcards", "review"]),
      ...(grammarQueue.length ? [] : ["grammar"]),
    ];
    replayCompletionKey.current = "";
    setCurrentRecallReplayPosition(null);
    setReplaySession({ day, vocabularyQueue: replayVocabularyQueues.flashcardQueue, recallQueue: replayVocabularyQueues.recallQueue, flashcardPosition: 0, cardPosition: 0, grammarQueue, grammarPosition: 0, completedSteps });
  ...
  function startCourse() {
    setProgress((current) => {
      ...
      const queues = buildRandomizedVocabularyQueues(buildDailyQueue(configured, vocabulary.length, cadenceNow));
  ...
  function nextShuffledTodayRecallQueue() {
    ...
    if (orderRegistry.date !== replayDate) {
      ...
      orderRegistry.signatures = new Set([queueSignature(progress.dailyQueue), queueSignature(progress.recallQueue)]);
    } else if (!orderRegistry.signatures.size) {
      orderRegistry.signatures.add(queueSignature(progress.dailyQueue));
      orderRegistry.signatures.add(queueSignature(progress.recallQueue));
    }
    const replayQueue = shuffleUnseenRecallQueue(progress.recallQueue, orderRegistry.signatures);
  ...
  function markFlashcardsComplete() {
    ...
    setCardPinyinOverride(null);
    if (replaySession) {
      setReplaySession((current) => current ? {
        ...current,
        flashcardPosition: current.vocabularyQueue.length,
        completedSteps: current.completedSteps.includes("flashcards") ? current.completedSteps : [...current.completedSteps, "flashcards"],
      } : current);
    } else {
  ...
  function reviewFlashcardsAgain() {
    ...
    if (replaySession) {
      setReplaySession((current) => current ? {
        ...current,
        vocabularyQueue: shuffleUnseenRecallQueue(current.vocabularyQueue, new Set([queueSignature(current.vocabularyQueue)])),
        flashcardPosition: 0,
      } : current);
    } else {
      setProgress((current) => ({
        ...current,
        dailyQueue: shuffleUnseenRecallQueue(current.dailyQueue, new Set([queueSignature(current.dailyQueue)])),
        flashcardPosition: 0,
      }));
    }
    setCardRevealed(false);
  ...
  function completeFlashcardCard() {
    ...
    recallAdvancingRef.current = true;
    if (replaySession) {
      setReplaySession((current) => current ? { ...current, flashcardPosition: current.flashcardPosition + 1 } : current);
    } else {
      setProgress((current) => ({ ...current, flashcardPosition: current.flashcardPosition + 1 }));
    }
    setCardRevealed(false);
    setCardPinyinOverride(null);
  }
  ...
  function completeVocabularyCard() {
    ...
    if (currentRecallReplayPosition !== null) {
      ...
      setCardRevealed(recallSpeakingRound);
      setToast(recallSpeakingRound ? "Speaking practice logged · automatic schedule unchanged" : "Extra recall logged · automatic schedule unchanged");
      return;
    }
    setProgress((current) => {
      ...
      if (next.cardPosition >= next.recallQueue.length) next = completeDailyStep(next, "review", 5, today);
      return next;
    });
    setCardRevealed(false);
  ...
  function studyWord(index: number) {
    ...
    setProgress((current) => {
      ...
      const existingPosition = queue.indexOf(index);
      const flashcardsWereComplete = current.daily.includes("flashcards");
      if (existingPosition >= 0) {
        return {
          ...current,
          flashcardPosition: flashcardsWereComplete ? existingPosition : Math.min(current.flashcardPosition, existingPosition),
          daily: flashcardsWereComplete ? current.daily.filter((step) => step !== "flashcards") : current.daily,
        };
      }
      const flashcardPosition = flashcardsWereComplete ? queue.length : current.flashcardPosition;
      queue.push(index);
      return {
        ...current,
        dailyQueue: queue,
    ...
    setCardRevealed(false);
    goToPractice("flashcards");
    setToast("Opened in flashcards · recall will test it afterward");
  }
  ...
  return (
    <main className="site-shell">
      ...
      <section className="section practice-section" id="practice">
        <div className="lesson-toolbar"><button onClick={() => navigate("today")}>{replaySession ? "← End replay" : "← Today’s plan"}</button><span>{replaySession ? studyDayLabel(replaySession.day.date) : `Mission ${activeMissionIndex + 1} · day ${missionPhase + 1}/3`} · step {practiceOrder.indexOf(practice) + 1} of 7</span></div>
        {replaySession && <div className="replay-banner"><span>↶</span><div><strong>Repeating {studyDayLabel(replaySession.day.date)}</strong><small>The same flashcards, recall test, grammar, reading, and mission are loaded. Today’s seven steps and course position stay untouched.</small></div><button onClick={() => navigate("today")}>Return to today</button></div>}
        <div className="section-heading two-column-heading"><div><span className="section-kicker">{replaySession ? "STUDY-DAY REPLAY" : "TODAY’S GUIDED LESSON"}</span><h2>{practiceLabels[practice]}</h2></div><p>{replaySession ? "Complete all seven steps again. Vocabulary cadence stays fixed; objective answers still update your accuracy." : "Focus on one step. Learn each scheduled card first, then prove recall without the answer."}</p></div>
        <div className="practice-tabs" role="tablist" aria-label={replaySession ? "Replay lesson steps" : "Today’s lesson steps"}>{([ ["flashcards", "01", "Cards"], ["recall", "02", "Recall"], ["grammar", "03", "Grammar"], ["listening", "04", "Listen"], ["builder", "05", "Build"], ["reading", "06", "Read"], ["speaking", "07", "Mission"] ] as [PracticeMode, string, string][]).map(([mode, number, label]) => { const step = dailySteps.find((item) => item.mode === mode); const done = step ? sessionDaily.includes(step.id) : false; const locked = mode === "recall" && !sessionDaily.includes("flashcards"); return <button key={mode} className={`${practice === mode ? "active" : ""} ${done ? "complete" : ""}`} onClick={() => setPractice(mode)} role="tab" aria-selected={practice === mode} disabled={locked} title={locked ? "Finish today’s flashcards first" : undefined}><span>{done ? "✓" : number}</span>{label}</button>; })}</div>

        <div className="practice-stage">
          {practice === "flashcards" && (
            <div className="flashcard-lab flashcard-study-lab">
              <div className="lab-instructions"><span className="micro-label">FULL-CARD STUDY · {Math.min(sessionFlashcardPosition + 1, sessionVocabularyQueue.length)} / {sessionVocabularyQueue.length}</span><h3>Learn first. Test second.</h3><p>Work through every word scheduled by your existing cadence. Start with the Chinese, then reveal the pronunciation, meaning, example sentence, audio, and speaking practice. Nothing is scored here.</p><div className="cadence-preview"><span>UP NEXT</span><strong>Recall test</strong><small>Same scheduled words · answers hidden</small></div><div className="lab-progress"><span style={{ width: `${queuePercent}%` }} /></div></div>
              {activeWord ? <>
                <div ref={studyCardRef} className={`study-card flashcard-study-card ${cardRevealed ? "revealed" : "recall-prompt-with-audio"}`}>
                  <button className="card-face-button" onClick={() => setCardRevealed(true)} disabled={cardRevealed} aria-label={cardRevealed ? "Vocabulary flashcard answer" : "Reveal vocabulary flashcard"}>
                    {!cardRevealed ? <>
                      <span className="card-caption">FLASHCARD FRONT · LOOK &amp; LISTEN</span>
                      <strong className={`hanzi-prompt ${activeWord.hanzi.length > 6 ? "very-long" : activeWord.hanzi.length > 3 ? "long" : ""}`}>{activeWord.hanzi}</strong>
                      <span className="flip-hint">Tap to reveal the complete card ↗</span>
                    </> : <>
                      <span className="card-caption">COMPLETE FLASHCARD · LISTEN &amp; SAY IT</span>
                      <strong className={`hanzi-prompt ${activeWord.hanzi.length > 6 ? "very-long" : activeWord.hanzi.length > 3 ? "long" : ""}`}>{activeWord.hanzi}</strong>
                      <span className={`pinyin-prompt ${promptLengthClass(activeWord.pinyin)}`}>{activeWord.pinyin}</span>
                      <span className="flashcard-meaning">{activeWord.meaning}</span>
                      {activeWord.example && <span className="card-example">
                        <small>EXAMPLE SENTENCE</small>
                        <strong lang="zh-CN">{activeWord.example}</strong>
                        <span>{activeWord.examplePinyin}</span>
                        <em>{activeWord.exampleTranslation}</em>
                      </span>}
                    </>}
                  </button>
                  <div className={`card-audio-actions ${cardRevealed ? "" : "prompt-audio-actions"}`}>
                    <button className="audio-link" onClick={() => speakVocabulary(activeWord)} aria-label={`Play pronunciation for ${activeWord.hanzi}`}>▶ Word</button>
                ...
                <div ref={studyCardRef} className={`study-card ${cardRevealed ? "revealed" : ""} ${recallPromptHasActions ? "recall-prompt-with-audio" : ""} ${recallFeedback === "correct" ? "recall-confirmation" : ""}`}>
                  <button className="card-face-button" disabled aria-label={speakingOnlyRecall ? "Vocabulary speaking card" : recallFeedback === "remediation" ? "Vocabulary answer review" : "Vocabulary recall prompt"}>
                    {recallFeedback === "correct" ? <>
                      <span className="card-caption">CORRECT · QUICK CONFIRMATION</span>
                      <strong className={`hanzi-prompt ${activeWord.hanzi.length > 6 ? "very-long" : activeWord.hanzi.length > 3 ? "long" : ""}`}>{activeWord.hanzi}</strong>
                      {showActivePinyin && <span className={`pinyin-prompt ${promptLengthClass(activeWord.pinyin)}`}>{activeWord.pinyin}</span>}
                      <span className="recall-confirmation-meaning">{activeWord.meaning}</span>
                      {recallExpansion === "example" && activeWord.example && <span className="card-example recall-confirmation-example"><small>EXAMPLE SENTENCE</small><strong lang="zh-CN">{activeWord.example}</strong>{showActivePinyin && <span>{activeWord.examplePinyin}</span>}<em>{activeWord.exampleTranslation}</em></span>}
                      <span className="flip-hint">Review the confirmation, then continue when ready</span>
                    </> : cardRevealed ? <>
                      <span className="card-caption">{speakingOnlyRecall ? "SPEAKING-ONLY ROUND" : "MISSED · STUDY THE COMPLETE ANSWER"}</span>
                      <strong className={`hanzi-prompt ${activeWord.hanzi.length > 6 ? "very-long" : activeWord.hanzi.length > 3 ? "long" : ""}`}>{activeWord.hanzi}</strong>
                      <span className={`pinyin-prompt ${promptLengthClass(activeWord.pinyin)}`}>{activeWord.pinyin}</span>
                      <span className="flashcard-meaning">{activeWord.meaning}</span>
                      {activeWord.example && <span className="card-example"><small>EXAMPLE SENTENCE</small><strong lang="zh-CN">{activeWord.example}</strong><span>{activeWord.examplePinyin}</span><em>{activeWord.exampleTranslation}</em></span>}
                    </> : <>
                      <span className="card-caption">RECALL TEST · {recallChallenge.mode.toUpperCase()}</span>
                  ...
                  {(cardRevealed || recallPromptHasActions) && <div className={`card-audio-actions ${recallPromptHasActions ? "prompt-audio-actions" : ""}`}>
                    {(cardRevealed || recallPromptHasAudio) && <button className="audio-link" onClick={() => speakVocabulary(activeWord)} aria-label={`Play pronunciation for ${activeWord.hanzi}`}>{recallPromptHasAudio ? "▶ Hear prompt" : "▶ Word"}</button>}
                    {recallPromptHasActions && <button className="audio-link" onClick={requestRecallHelp}>I don’t recall</button>}
                    {activeWord.example && cardRevealed && <button className="audio-link" onClick={() => speak(activeWord.example)} aria-label={`Play example sentence ${activeWord.example}`}>▶ Example</button>}
                  </div>}
                </div>
                {recallFeedback === "correct" && <div className="recall-confirmation-controls" aria-label="Correct answer options"><button onClick={() => speakVocabulary(activeWord)}>▶ Hear word</button><button onClick={() => expandRecallConfirmation("speaking")}>● Practice speaking</button>{activeWord.example && <button onClick={() => expandRecallConfirmation("example")}>例 See example</button>}<button onClick={() => setCardPinyinOverride(!showActivePinyin)}>{showActivePinyin ? "Hide pinyin" : "Show pinyin"}</button><button className="confirmation-next" onClick={advanceAfterRecallConfirmation}>Continue to next card →</button></div>}
                {recallFeedback === "remediation" && <p className="recall-remediation-note">{recallResult}</p>}
                {(speakingOnlyRecall || recallFeedback === "remediation" || recallExpansion === "speaking") && <RecallSpeechPractice key={`recall-${selectedLevel}-${activeWordIndex}`} word={activeWord} showPinyin={recallFeedback === "remediation" || speakingOnlyRecall || showRecallPinyin} onAttempt={(correct) => setProgress((current) => recordSkillAttempt(current, "speaking", correct, today))} />}
          ...
          {practice === "grammar" && <div className="grammar-lesson">
            ...
            <div className={`grammar-teach-card grammar-stage-${dailyGrammarStage} ${dailyGrammar.example ? "" : "formula-recall"}`}>
              ...
              {dailyGrammarStage === "learn" ? <>
                <span>{dailyGrammar.label} · {getGrammarLesson(dailyGrammar).unit}</span>
                <h4 className="grammar-stage-title">Understand the pattern</h4>
                <code>{dailyGrammar.formula}</code>
                <div className="grammar-notice"><strong>The exact target</strong><p>The formula above is the grammar being learned. The builder below keeps that target intact and adds only the subject, predicate, object, or context needed for a complete thought.</p></div>
                <GrammarExplanation point={dailyGrammar} />
        ...
        {practice === "speaking" && completedMissionSession && <div className="correction-lab"><div><span className="micro-label">AUTOMATIC CORRECTION LOOP</span><h3>{activeCorrection ? `${activeCorrections.length} miss${activeCorrections.length === 1 ? "" : "es"} ready to repair` : "Today’s misses are repaired."}</h3><p>{activeCorrection ? "Answer correctly now and once more tomorrow. This extra practice never changes the fixed vocabulary cadence." : "Anything you miss later will appear here automatically."}</p></div>{activeCorrection && <div className="correction-card"><span>{skillLabels[activeCorrection.skill]}</span><strong>{activeCorrection.prompt}</strong>{activeCorrectionAudio && <div className="correction-audio-actions"><button onClick={() => activeCorrectionAudioWord ? speakVocabulary(activeCorrectionAudioWord) : speak(activeCorrectionAudio)}><span>▶</span> Play correction audio</button><button onClick={() => activeCorrectionAudioWord ? speakVocabulary(activeCorrectionAudioWord, 0.62) : speak(activeCorrectionAudio, 0.62)}>Play slower</button></div>}<div>{activeCorrection.options.map((option) => <button key={option} onClick={() => answerCorrection(option)}>{option}</button>)}</div>{correctionResultFor === activeCorrection.id && correctionResult && <p className={correctionResult.startsWith("Correct") ? "correct" : ""}>{correctionResult}<small>{activeCorrection.explanation}</small></p>}</div>}</div>}
        <div className={`lesson-next-bar ${practice === "speaking" ? "lesson-finish-bar" : ""}`}><button onClick={() => navigate("today")}>{practice === "speaking" && completedMissionSession ? "Return to plan" : "Save & return to plan"}</button>{practice === "speaking" ? <><div><span>{replaySession ? "REPLAY" : activeCorrections.length > 0 && completedMissionSession ? "FINAL CHECK" : "FINISH TODAY"}</span><strong>{!completedMissionSession ? "Complete the mission above" : replaySession ? "Replay complete" : activeCorrections.length > 0 ? `${activeCorrections.length} correction${activeCorrections.length === 1 ? "" : "s"} ready` : "All required work complete"}</strong></div><button className="next-step-button finish-day-button" disabled={!completedMissionSession} onClick={() => replaySession ? navigate("today") : finishStudyDay()}>{!completedMissionSession ? "Finish mission first" : replaySession ? "Finish replay →" : activeCorrections.length > 0 ? "Clear corrections →" : "Finish day →"}</button></> : <><div><span>{(practice === "flashcards" || practice === "recall") && !currentPracticeComplete ? "FINISH THIS STEP" : "UP NEXT"}</span><strong>{(practice === "flashcards" || practice === "recall") && !currentPracticeComplete ? practiceLabels[practice] : practiceLabels[nextPractice]}</strong></div><button className="next-step-button" disabled={(practice === "flashcards" || practice === "recall") && !currentPracticeComplete} onClick={() => setPractice(nextPractice)}>{(practice === "flashcards" || practice === "recall") && !currentPracticeComplete ? "Complete cards above" : "Continue →"}</button></>}</div>
      </section>
      )}
      ...
      {appView === "progress" && (
        <section className="section progress-section app-view" id="progress">
          ...
          <div className="app-about"><div><span className="brand-mark">声</span><div><strong>SHĒNGTÚ</strong><p>Hear it. Say it. Own it.</p></div></div><div><span>SOURCES</span><a href="https://www.chinesetest.cn/syllabus" target="_blank" rel="noreferrer">Official HSK 3.0 ↗</a><a href="https://hsk.cn-bj.ufileos.com/3.0/%E6%96%B0%E7%89%88HSK%E8%80%83%E8%AF%95%E5%A4%A7%E7%BA%B2%EF%BC%88%E8%AF%8D%E6%B1%87%E3%80%81%E6%B1%89%E5%AD%97%E3%80%81%E8%AF%AD%E6%B3%95%EF%BC%89.pdf" target="_blank" rel="noreferrer">2025 syllabus PDF ↗</a><a href="https://cc-cedict.org/editor/editor.php?handler=Download" target="_blank" rel="noreferrer">English glosses · CC-CEDICT ↗</a><a href="./audio/ATTRIBUTION.txt" target="_blank" rel="noreferrer">Recorded pronunciation credits ↗</a></div><small>Independent learning tool. Not affiliated with Chinese Test International.</small></div>
        </section>
      )}
      </div>
